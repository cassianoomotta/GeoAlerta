...
import { Boxes, Plus, Archive, RefreshCw, Trash2 } from "lucide-react";
...
export default function RecursosPage() {
  ...
  const resetForm = () => {
    setName(""); setCategory(CATEGORIES[0]); setQuantity("0"); setUnit("un"); setExpiry("");
    setShelterId(""); setStatus(STATUSES[0]); setNotes("");
  };

  const openEdit = (r: Resource) => {
    setEditingId(r.id);
    setName(r.name); setCategory(r.category); setQuantity(String(r.quantity)); setUnit(r.unit);
    setExpiry(r.expiry_date?.slice(0, 10) || ""); setShelterId(r.shelter_id || ""); setStatus(r.status); setNotes(r.notes || "");
    setShowForm(true);
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      <PageHeader
        ...
        subtitle="Itens, quantidades, validade e movimentações estratégicas"
        action={
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left min-w-[820px] border-collapse">
              <thead className="bg-white/[0.03] sticky top-0 z-10">
                <tr>
                  {["Item", "Categoria", "Qtd", "Validade", "Abrigo", "Status", ""].map((h) => (
                ...
                {items.map((r) => (
                  <tr key={r.id} className="hover:bg-white/[0.03] transition-colors">
                    <td className="p-3">
                      ...
                      {r.notes && <span className="text-[11px] text-slate-500">{r.notes}</span>}
                    </td>
                    <td className="p-3 text-xs text-slate-300">{r.category}</td>
                    <td className="p-3 text-sm font-bold text-white">{fmtQty(r.quantity)} <span className="text-[10px] text-slate-500">{r.unit}</span></td>
                    <td className="p-3 text-xs text-slate-400">{r.expiry_date ? fmtDate(r.expiry_date) : "—"}</td>
                    <td className="p-3 text-xs text-slate-300">{shelters.find((s) => s.id === r.shelter_id)?.name || "—"}</td>
      ...
      {Object.keys(movements).length > 0 && (
        <Modal onClose={() => setMovements({})} title="Movimentações do recurso">
          ...
          <p className="text-xs text-slate-500 mt-4">
            O detalhamento por movimentação (data, quem, motivo) será registrado na tabela <code>resource_movements</code>.
          </p>
        </Modal>
      )}
      ...
      {showForm && (
        <Modal onClose={() => setShowForm(false)} title={editingId ? "Editar Recurso" : "Novo Recurso"}>
          <form onSubmit={onSubmit} className="space-y-4">
            <Field label="Nome do item" required><input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="ex: Água mineral 500ml" required /></Field>
            <Field label="Categoria">
              <select className={inputCls} value={category} onChange={(e) => setCategory(e.target.value)}>{CATEGORIES.map((c) => <option key={c} className="bg-slate-900 text-white" value={c}>{c}</option>)}</select>
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Quantidade" required><input type="number" min="0" className={inputCls} value={quantity} onChange={(e) => setQuantity(e.target.value)} required /></Field>
              <Field label="Unidade"><input className={inputCls} value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="un, kg, L, cx" /></Field>
            </div>
            <Field label="Data de validade"><input type="date" className={inputCls} value={expiry} onChange={(e) => setExpiry(e.target.value)} /></Field>
            <Field label="Abrigo vinculado (opcional)">
              <select className={inputCls} value={shelterId} onChange={(e) => setShelterId(e.target.value)}>
                <option value="" className="bg-slate-900 text-white">Nenhum</option>
